...
4. Add test recipients (Meta test numbers or the client's testers) and set the non-production allowlist: `PUT /api/settings/outbound.allowed_recipients` `{ "value": ["2637…"] }` so no one else can be messaged from a test environment.
   **This allowlist also governs the built-in simulator.** Once it is set, any number not on it is refused at dispatch with `RECIPIENT_NOT_ALLOWED` and the row goes to `permanent_failure`, including the simulator's default test number. The conversation still runs and the reply is still produced — the simulator now says "not delivered" and names the reason on each reply, rather than showing an answer that never left. To exercise delivery end to end, add the number you are simulating. Add only a number **you control**: on a deployment with real credentials the platform will genuinely dial it.
