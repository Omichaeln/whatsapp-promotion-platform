...
| 012_drop_unused_indexes.sql | contract (index drops only) | drops `idx_entries_draw` and `idx_receipts_processing`, both measured as never chosen by a query plan on this schema (v2 filters entries by `period_code`, and a status-only receipt lookup prefers `idx_receipts_status`, which also satisfies the ORDER BY). No table, column or row of application data is touched. Rollback: the previous release runs correctly without them; re-create either index from the file only if a plan regression appears |
