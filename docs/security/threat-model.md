...
| Personal data | leakage via logs/exports/API | masks on phones and identity numbers; encrypted identity; audited reveal; CSV formula neutralisation; watermarked, role-gated exports; request logging off at the default `LOG_LEVEL=info` — at `debug` the request path is logged verbatim, so phone-bearing paths put full numbers in the log (`docs/release/production-checklist.md`) | T-30, T-31 |
