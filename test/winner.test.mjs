...
async function buildPublishedDraw(ctx) {
...
describe("winners + claims lifecycle (DEF-03, REQ-20/22)", () => {
  it("publishing a draw materialises winners + claims + queued notifications, idempotently", async () => {
    ...
    assert.equal(r.created, 3, "one winner per entry");
    assert.equal(r.idempotent, false);
    assert.equal(winners.listByDraw(f.id).length, 3);
    ...
    assert.equal(queued, 3, "3 winner notifications queued");
  ...
  it("public winners view exposes disclosure fields only (no full phone / identity)", async () => {
    ...
    winners.materialise(f.id);
    winners.listByDraw(f.id).forEach((w, i) => winners.transition(w.id, { status: i === 0 ? "collected" : "notified", actorId: "ops" }));
