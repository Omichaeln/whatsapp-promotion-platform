...
function Gate({ brand, onDone }) {
  ...
  const submit = async () => {
    ...
    if (out.ok) onDone();
    else { setBad(true); setToken(""); }
  };
  return (
    <div className="gate">
      <div className="gate-card frame">
        <div className="overline">{brand}</div>
        <div style={{ fontSize: 12.5, color: "var(--text-secondary)", marginBottom: 12, lineHeight: 1.6 }}>Enter the access token from the desk's .env file.</div>
        <input className="field" type="password" value={value} onChange={e => setValue(e.target.value)} onKeyDown={e => { if (e.key === "Enter") submit(); }} placeholder="Access token" style={{ width: "100%" }} autoFocus />
        {bad && <div className="notice" style={{ marginTop: 8 }}>That token was not accepted.</div>}
        <div style={{ marginTop: 14 }}><button className="btn primary" onClick={submit} disabled={!value.trim()}>Open the desk</button></div>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    api("/api/config").then(out => {
      ...
      if (out.data.locked && !getToken()) setGate(true);
    });
