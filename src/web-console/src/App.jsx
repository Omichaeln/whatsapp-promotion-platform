...
function CampaignOutlets({ id, data, me, onChange }) {
  ...
  // Remove deletes exactly ONE membership row (DELETE .../outlets/:outletId).
  // It used to full-replace the membership with the PUT, and the list it built
  // that body from is filtered to active master records, so two rows were
  // deleted that the operator never asked about and could not see: every member
  // whose master outlet is inactive vanished from the campaign, and every
  // surviving member's campaign window (active_from/active_to, which
  // winner-service reads to refuse a prize collection outside it) was reset to
  // always open. A targeted delete touches nothing but the row clicked.
  const remove = (o) => a.run(() => api(`/api/campaigns/${id}/outlets/${encodeURIComponent(o.id)}`, { method: "DELETE" }), { confirm: `Remove ${o.outlet_code} (${o.branch}) from this campaign? Consumers will no longer see it in the outlet menu.\n\nOnly this outlet's membership is removed; the master record, and every other outlet's membership and collection window, are left as they are.` }).then((r) => { if (r?.ok) { reload(); onChange(); } });
  return <>
    <Card title={`Participating outlets (${rows.length})`} right={<a className="link-btn" href="/api/outlets/export.csv" onClick={(e) => { e.preventDefault(); api("/api/outlets/export.csv", {}).then(() => window.open(`/api/outlets/export.csv`)); }}>Export CSV</a>}>
      ...
      <div className="sub">Removing takes the outlet out of this campaign only; the master record (and its aliases) is kept, and no other outlet\u2019s membership or collection window is touched. Outlets whose master record is inactive are not listed here.</div>
    </Card>
...
function outletBody(f, cur) {
  ...
  return {
    ...
    // retailer_code has no field in this form but is part of the row the upsert
    // replaces: dropping it turned reset-sample.mjs's `delete ... where
    // retailer_code='TEST'` into a no-op (an edited sample outlet became
    // permanent) and removed half of activation.mjs's OUTLETS_SAMPLE gate.
    retailer_code: f.retailer_code !== undefined ? f.retailer_code : (cur?.retailer_code ?? undefined),
...
/**
 * outlets.active_from / active_to are compared as strings (winner-service and
 * the outlet menu both slice(0,10) and compare), so "2026-1-1" silently sorts
 * before "1970-01-01". The CSV path rejects an unparseable date; this form was
 * the first to expose the two columns as free text and validated nothing.
 */
const ISO_DAY = /^\d{4}-\d{2}-\d{2}([T ].*)?$/;
const badDay = (x) => { const t = String(x ?? "").trim(); return t !== "" && (!ISO_DAY.test(t) || Number.isNaN(Date.parse(t.slice(0, 10)))); };
function Outlets() {
  const [l, reload] = useApi("/api/outlets"); const [f, setF] = useState({}); const a = useAction();
  const dateErr = ["active_from", "active_to"].filter((k) => badDay(f[k]));
  ...
  return <div className="promo-page">
    <Card title="Add / update outlet (master)" right={f.outlet_code ? <Btn ghost small onClick={() => setF({})}>Clear / new outlet</Btn> : null}>
      <div className="row">{[["outlet_code", "Code"], ["retailer", "Retailer"], ["branch", "Branch"], ["town", "Town"], ["province", "Province"], ["aliases", "Aliases (; separated)"], ["active_from", "Active from"], ["active_to", "Active to"]].map(([k, l2]) => <Field key={k} label={l2}><Input value={shown(k, "") ?? ""} onChange={(e) => setF({ ...f, [k]: e.target.value })} /></Field>)}
        ...
        <Btn disabled={a.busy || !f.outlet_code || dateErr.length > 0} onClick={save}>Save</Btn></div>
      {dateErr.length > 0 && <div className="notice">{dateErr.join(" and ")} must be a calendar date written as YYYY-MM-DD — the collection window is compared as text, so any other spelling silently widens or closes it.</div>}
...
function Draws({ me }) {
  ...
  return <div className="promo-page"><a.Msg />
    <Card title="Prepare a draw (cutoff barrier → freeze → execute → independent approval → publish)">
      <div className="row"><Field label="Campaign"><select className="field" value={cid || ""} onChange={(e) => { setPicked(e.target.value); setPid(""); setBarrier(null); }}>{/* value="" with no matching option makes the browser display the first
            campaign in the list while cid is null and nothing is loaded — the
            view then shows a campaign it is not targeting. The placeholder is
            unconditional so the select can always render its own value. */}
          {cid ? null : <option value="">{all.length ? "— choose a campaign —" : "— none —"}</option>}{all.map((c) => <option key={c.id} value={c.id}>{c.code} — {c.name} ({c.status})</option>)}</select></Field><Field label="Period"><select className="field" value={pid} onChange={(e) => { setPid(e.target.value); setBarrier(null); }}><option value="">— choose —</option>{(periods.data?.periods || []).map((p) => <option key={p.id} value={p.id}>{p.code} — {p.label} ({p.status})</option>)}</select></Field><Btn ghost disabled={!pid} onClick={check}>Check barrier</Btn>{can("draw_officer") && <Btn disabled={!barrier?.ok || a.busy} onClick={() => a.run(() => api("/api/draws", { method: "POST", body: { campaign_id: cid, period_id: pid } }), { confirm: "Freeze the candidate pool and commit randomness for this period?" }).then(() => { setBarrier(null); reload(); })}>Freeze candidates</Btn>}</div>
...
/* ===================== winners ===================== */
function Winners({ me }) {
  const [l, reload] = useApi("/api/winners"); const [sel, setSel] = useState(null); const a = useAction();
  // The collection select has to offer exactly what winner-service
  // assertCollectionPoint accepts: a campaign_outlets row for THIS winner's
  // campaign, with the campaign-level collection flag set and today inside the
  // membership window. Sourcing it from the master /api/outlets list offered
  // branches the campaign had excluded — and, after a Remove on the campaign
  // outlets tab, branches with no membership at all — which the server then
  // refuses at 'accepted'. /api/winners already carries the campaign id.
  const selRow = (l.data?.winners || []).find((w) => w.id === sel) || null;
  const campId = selRow?.campaign_id || null;
  const [outlets] = useApi(campId ? `/api/campaigns/${campId}/outlets` : "/api/whoami", [campId]);
  ...
  // They are also the inputs of ONE winner. Held in a single object shared by
  // the whole list, evidence typed for winner A stayed in the field when winner
  // B was opened, so "Mark verified" sent A's ID-check note as B's: a claim row
  // and winner.verified audit payload that read as complete while naming the
  // wrong person's ID check. The same leak pre-armed A's collection point and
  // fulfilment reference for B. Values are tagged with the winner they were
  // typed for and ignored for any other.
  const [claimState, setClaimState] = useState({});
  const claim = claimState.winner === sel ? claimState : {};
  const setClaim = (next) => setClaimState({ ...next, winner: sel });
  ...
  return <div className="promo-page"><a.Msg />
    ...
    {sel && detail.data?.winner && (() => { const w = detail.data.winner; const today = new Date().toISOString().slice(0, 10); const collect = (outlets.data?.outlets || []).filter((o) => o.collection_enabled && o.campaign_collection_enabled && o.active && String(o.member_from || "1970-01-01").slice(0, 10) <= today && String(o.member_to || "9999-12-31").slice(0, 10) >= today); const outletPick = () => <Field label="Assign collection point"><select className="field" value={claim.outlet || ""} onChange={(e) => setClaim({ ...claim, outlet: e.target.value })}><option value="">— choose a collection point —</option>{collect.map((o) => <option key={o.id} value={o.id}>{o.retailer} — {o.branch}, {o.town}</option>)}</select></Field>; return <Card title={`Winner ${w.display_name} — ${w.status}`} right={<Btn ghost small onClick={() => setSel(null)}>Close</Btn>}>
      ...
      {can && <div className="row" style={{ marginTop: 10 }}>
        ...
        {/* These three write the operator's words into the claim trail and the
            winner audit payload. As prompt() dialogs they recorded "" whenever
            the operator pressed Escape — a disputed/ineligible/replaced winner
            with no stated reason — so they take the same required inline field
            as the evidence above and stay disabled until it is filled. */}
        {!["collected", "replaced"].includes(w.status) && <><Btn ghost onClick={() => tr(w.id, "unreachable", { expected_version: w.row_version })}>Unreachable</Btn><Btn ghost onClick={() => tr(w.id, "declined", { expected_version: w.row_version })}>Declined</Btn><Field label="Claim reason (dispute / ineligible / replacement)"><Input value={claim.reason || ""} onChange={(e) => setClaim({ ...claim, reason: e.target.value })} placeholder="why — recorded in the claim trail" /></Field><Btn ghost disabled={a.busy || !(claim.reason || "").trim()} onClick={() => tr(w.id, "disputed", { expected_version: w.row_version, note: claim.reason.trim() }).then((r) => { if (r?.ok) setClaim({}); })}>Disputed</Btn><Btn danger disabled={a.busy || !(claim.reason || "").trim()} onClick={() => tr(w.id, "ineligible", { expected_version: w.row_version, reason: claim.reason.trim() }, "Mark ineligible?").then((r) => { if (r?.ok) setClaim({}); })}>Ineligible</Btn><Btn danger disabled={a.busy || !(claim.reason || "").trim()} onClick={() => tr(w.id, "replaced", { expected_version: w.row_version, reason: claim.reason.trim() }, "Replace with the next approved alternate? This is audited and cannot be undone.").then((r) => { if (r?.ok) setClaim({}); })}>Replace with alternate</Btn></>}
      </div>}
      {canPublish && <div className="row" style={{ marginTop: 10 }}>
        ...
        {w.publication_state === "published" && <><Field label="Withdrawal reason (audited)"><Input value={claim.unpublish || ""} onChange={(e) => setClaim({ ...claim, unpublish: e.target.value })} placeholder="why the public list is being corrected" /></Field><Btn ghost disabled={a.busy || !(claim.unpublish || "").trim()} onClick={() => a.run(() => api(`/api/winners/${w.id}/unpublish`, { method: "POST", body: { reason: claim.unpublish.trim() } })).then((r) => { both(); if (r?.ok) setClaim({}); })}>Withdraw publication</Btn></>}
      </div>}
...
function Access({ me }) {
  ...
  return <div className="promo-page"><a.Msg />
    ...
    <Card title="Staff accounts">{l.error ? <Err e={l.error} /> : null}<Table empty={l.error ? `Not shown: ${l.error}` : "Nothing here yet."} cols={["Email", "Name", "Roles", "MFA", "Status", "Temp pw", "Last login", ""]} rows={l.data?.users} render={(u) => <tr key={u.id}><td>{u.email}</td><td>{u.name}</td><td className="sub">{u.roles.join(", ")}</td><td>{u.mfa_enabled ? <Chip s="ok" /> : <Chip s="warning" />}</td><td><Chip s={u.status} /></td><td>{u.must_change_password ? "yes" : ""}</td><td className="sub">{fmt(u.last_login_at)}</td><td>{me.roles.includes("platform_admin") && u.id !== me.id && <><Btn ghost small onClick={() => { const r = prompt("Roles (comma separated)", u.roles.join(",")); if (r != null) a.run(() => api(`/api/users/${u.id}`, { method: "PATCH", body: { roles: r.split(",").map((x) => x.trim()).filter(Boolean) } })).then(reload); }}>Roles</Btn> <Btn ghost small onClick={() => a.run(() => api(`/api/users/${u.id}`, { method: "PATCH", body: { status: u.status === "active" ? "disabled" : "active" } }), { confirm: `${u.status === "active" ? "Disable" : "Enable"} ${u.email}? Sessions are revoked.` }).then(reload)}>{u.status === "active" ? "Disable" : "Enable"}</Btn> <Btn ghost small onClick={async () => { const r = await a.run(() => api(`/api/users/${u.id}/reset-password`, { method: "POST" }), { confirm: `Reset password for ${u.email}?` }); if (r?.ok) setTemp({ user: u, temporaryPassword: r.data.temporaryPassword }); }}>Reset pw</Btn></>}</td></tr>} /></Card>
