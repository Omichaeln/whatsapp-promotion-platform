...
// deploy discards the container layer with every row in it.
{ const v = dataVolumeStatus(cfg); ok("data volume mounted", !v.checked || v.ok, v.checked ? `${v.dir} ${v.ok ? "marked" : `has no marker (VOLUME_INIT=true once to provision${v.dbExists ? "; the database already exists there" : ""})`}` : "VOLUME_PATH not set (check disabled)", v.checked); }
